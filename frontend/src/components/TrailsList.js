...
export default function TrailsList({ trails }) {
    if (!trails || trails.length === 0) {
        return <div>No trails found. Try another state code.</div>;
    }
                ...
                <div className="col-md-6 mb-3" key={idx}>
                    <div className="card h-100">
                        ...
                        <div className="card-body">
                            <h5 className="card-title">{t.name || t.park_name}</h5>
                        ...
                        <div className="card-footer text-muted">
                            {t.state ? <small>{t.state}</small> : null}
                            {t.latitude && t.longitude ? <span className="ms-3">Lat: {t.latitude}</span> : null}
                        </div>
                    </div>
                </div>
            ))}
